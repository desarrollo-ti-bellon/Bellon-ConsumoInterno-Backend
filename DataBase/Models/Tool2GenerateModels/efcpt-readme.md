...
1. Register your DbContext class in your "Program.cs" file.
    ...
    ```csharp
    builder.Services.AddDbContext<bellonappsContext>(
...
2. Add "ConnectionStrings" to your configuration file (secrets.json, appsettings.Development.json or appsettings.json).
    ...
    {
        "ConnectionStrings": {
            "DefaultConnection": "data source=tcp:bellonapps.database.windows.net,1433;initial catalog=bellonapps;persist security info=False;multiple active result sets=False;connect timeout=30;encrypt=True;trust server certificate=True;command timeout=300"
        }
    }
    ```
